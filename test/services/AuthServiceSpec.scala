...
class AuthServiceSpec extends PlaySpec
  ...
  "AuthService" should {
    ...
    "register a new user successfully" in {
      ...
      // When
      val resultFuture = authService.register("newuser", "new@example.com", "newpassword")
      val result = Await.result(resultFuture, 5.seconds)
      
      // Then
      result.isRight mustBe true
      result.getOrElse(fail("Expected Right but got Left")).username mustBe "newuser"
      result.getOrElse(fail("Expected Right but got Left")).email mustBe "new@example.com"
    ...
    "reject registration with existing username" in {
      ...
      // When
      val resultFuture = authService.register("testuser", "new@example.com", "newpassword")
      val result = Await.result(resultFuture, 5.seconds)
    ...
    "reject registration with existing email" in {
      ...
      // When
      val resultFuture = authService.register("newuser", "test@example.com", "newpassword")
      val result = Await.result(resultFuture, 5.seconds)
