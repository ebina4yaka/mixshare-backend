...
class TokenBlacklistServiceSpec extends PlaySpec
  ...
  val tokenBlacklistService = new TokenBlacklistService(mockRedisService, config)
  ...
  val testToken = JWT.create()
    ...
    .withExpiresAt(new java.util.Date(System.currentTimeMillis() + 3600 * 1000)) // 1 hour
    .withJWTId("test-token-id")
    .sign(algorithm)
  
  val tokenWithoutJti = JWT.create()
    .withIssuer(configData("jwt.issuer"))
    .withSubject("1")
    .withClaim("username", "testuser")
    .withClaim("tokenType", "access")
  ...
  "TokenBlacklistService" should {
    ...
    "handle Redis failure when blacklisting" in {
      // Setup
      when(mockRedisService.setEx(anyString(), anyString(), anyLong()))
        .thenReturn(scala.util.Failure(new RuntimeException("Redis connection failed")))
