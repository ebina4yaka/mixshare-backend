...
package object models {
  ...
  implicit val zonedDateTimeFormat: Format[ZonedDateTime] = new Format[ZonedDateTime] {
      ...
      try {
        ...
        if (dateStr.contains(" ") && dateStr.matches(".*\\+\\d{2}$")) {
          try {
            ...
            val regex = "(\\d{4}-\\d{2}-\\d{2}) (\\d{2}:\\d{2}:\\d{2}\\.\\d+)\\+(\\d{2})".r
            dateStr match {
              case regex(date, time, zone) => 
                // Parse the local date-time part
                val formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss.SSSSSS")
                val localDateTime = LocalDateTime.parse(s"$date $time", formatter)
                // Convert zone offset "+09" to hours
                val zoneOffsetHours = zone.toInt
                // Create ZonedDateTime with the offset but no zone ID
                val result = ZonedDateTime.of(localDateTime, ZoneOffset.ofHours(zoneOffsetHours))
                // Format and parse to remove any zone ID
                val cleanResult = ZonedDateTime.parse(result.format(zonedDateTimeFormatter), zonedDateTimeFormatter)
            ...
            case e: Exception =>
              JsError(s"Failed to manually parse datetime '$dateStr': ${e.getMessage}")
          }
          ...
          try {
            ...
            val cleanZdt = ZonedDateTime.parse(zdt.format(zonedDateTimeFormatter), zonedDateTimeFormatter)
            JsSuccess(cleanZdt)
          } catch {
            case e: Exception =>
              JsError(s"Failed to parse datetime with zone ID: ${e.getMessage}")
          }
        } else {
        ...
        case e: Exception =>
          JsError(s"Failed to extract string value from JSON: ${e.getMessage}")
      }
    }
    ...
    def writes(dateTime: ZonedDateTime): JsValue = {
      ...
      val cleanDateTime = ZonedDateTime.parse(dateTime.format(zonedDateTimeFormatter), zonedDateTimeFormatter)
